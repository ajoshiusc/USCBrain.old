...
ind = (vol_img == 150) | (vol_img == 151)  # selects precentral from BCI atlas
Xc = X[ind]
Yc = Y[ind]
Zc = Z[ind]
...
vol_lab = ni.load_img(USCBrainbase+'/BCI-DNI_brain.label.nii.gz')
vol_img = vol_lab.get_fdata()

# here make sure that hanns labels are not modified TBD
vol_img[ind] = t.labels

new_img = ni.new_img_like(vol_lab, vol_img)
new_img.to_filename(USCBrainbaseLatest +
                    '/BCI-DNI_brain.label.precent_corr.nii.gz')
