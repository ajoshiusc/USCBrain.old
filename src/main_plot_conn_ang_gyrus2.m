...
%plot_outline_labels(s,'k'); 
colormap gray; caxis([40/3,40])
view(-45,15); camlight;material dull;lighting phong;
ax = gca;               % get the current axis
ax.Clipping = 'off';    % turn clipping off
camlight;
set(gcf, 'Position', get(0,'Screensize'));%zoom(2); % Maximize figure.
saveas(h,'left_ang_gyrus_Grayordinates_freq2.png');
%%
...
s.attributes(sl.labels~=226+1)=0;
h=figure;
...
ax.Clipping = 'off';    % turn clipping off
set(gcf, 'Position', get(0,'Screensize'));%zoom(2); % Maximize figure.
saveas(h,'conn_ang_gyrus21_1.png');
view(-90,0); camlight;
saveas(h,'conn_ang_gyrus21_2.png');
